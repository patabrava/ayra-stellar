...
- Before manual Vercel production deploys, integrate the latest `origin/main` public-page changes into the source branch, then commit and push the deployed source; never let a stale feature branch restore older navigation, responsive markup, or hard-coded project media.
- Payout destinations that require Stellar memos must persist and display the verified memo and pass it unchanged to SDP; an address-only registry row is not payment-ready.
